...
program:(VOID|INT) MAIN '(' ')' '{'  statements[1] (RETURN (logic_arith_expression)? ';')? '}'
	;
...
logic_arith_expression returns [int value]
		  ...
		  |LS_OP c = arith_expression   {$value = ($value<$c.value) ? 1 : 0;}
		  |EQ_OP d = arith_expression   {$value = ($value==$d.value) ? 1 : 0;}
		  |LE_OP e = arith_expression   {$value = ($value<=$e.value) ? 1 : 0;}
		  |GE_OP f = arith_expression   {$value = ($value>=$f.value) ? 1 : 0;}
		  |NE_OP g = arith_expression   {$value = ($value!=$g.value) ? 1 : 0;}
		  |AND  h = arith_expression    {$value = (($value!=1)||($h.value !=1) )? 0 : 1;}
		  |OR  i = arith_expression	{$value = (($value ==1)|| ($i.value ==1 ) )? 1 : 0;}
...
statement[int flag]: Identifier '=' logic_arith_expression ';'
	   {
	   	if($flag>=1){
			Integer v = (Integer)memory.get($Identifier.text);
			if (v != null)
         ...
         |IF '(' logic_arith_expression')' 
         if_statements[(($logic_arith_expression.value>=1)&&($flag>=1))?1:0] ((ELSE) => ELSE else_statements[(($logic_arith_expression.value>=1)||($flag<1))?1:0]|  )
	 |FOR'('forcondition';' forcondition';'forcondition ')'if_statements[1]
	  
	 |logic_arith_expression ';'
	 { System.out.println($logic_arith_expression.value); }
	
	 |WHILE '(' logic_arith_expression ')' if_statements[1]
...
else_statements [int flag]: 
		statement[($flag<1)? 1 : 0]
                | '{' statements[($flag<1)? 1 : 0] '}'
